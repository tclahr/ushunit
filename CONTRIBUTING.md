...
We welcome contributions to the ushunit project in many forms, and there's always plenty to do!
...
If you are a user and you find a bug, please submit an [issue](https://github.com/tclahr/ushunit/issues). Please try to provide sufficient information for someone else to reproduce the issue. One of the project's [maintainers](MAINTAINERS.md) should respond to your issue soon.

Please search within our [already-reported bugs](https://github.com/tclahr/ushunit/issues) before raising a new one to make sure you're not raising a duplicate.
...
- You can use the [Shell Style Guide](https://google.github.io/styleguide/shellguide.html) as a reference. Keep in mind that ushunit is meant to run on any bourne shell, so some features like arrays, command substitution ```$(command)``` and tests ```[[ ... ]]``` are not fully supported by most of them.
